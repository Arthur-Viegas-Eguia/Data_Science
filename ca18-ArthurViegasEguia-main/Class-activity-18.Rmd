...
covid_data <- read_csv(table_usafacts[2]) %>% filter(State == "MN") %>% 
  ...
  slice(-1) %>% 
  pivot_longer(-1, names_to = "Dates", values_to = "Cases") %>%  #now we have longer data
  janitor::clean_names() %>%  #we vlean the names, make them data frame ish
  mutate(county_name = str_remove(county_name, " County"),  #Do some data wrangling
         dates = ymd(dates),
...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      sliderInput(inputId = "monthInput",
                label = "Month",
                min = 1, 
                max = 12,
                value = c(1, 7)),
      radioButtons("yearInput",
                   "Year",
                   2020:2024,
                   "2023"),
      selectInput("county",
    ...
    mainPanel("Your results go here")
  )
)
...
server <- function(input, output) {
    ...
    output$plot <- renderPlot({
      ...
      ggplot(filtered_data(), aes(x = dates, y = cases)) +
