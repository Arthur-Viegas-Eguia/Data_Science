...
```{r}
dat1 <- babynames %>% filter(name %in% c("Apple", "Yoroi", "Ada")) #%>% pull(name) %>% unique()
```
...
```{r}
dat1 %>% mutate_if(is.character, toupper) %>% rename(count = n)
```

### e. Change all the column names to upper case in the previous problem.

```{r}
babynames %>% rename_at(vars(1:5), toupper)
...
*answer:* #1 Selects all the numeric columns, which is amount

\#2 Renames all the column names to uppercase


\#3 changes all column names that contains character data to uppercase (the column name will be in uppercase)

\#4 Changes the column name that contains it to uppercase
...
```{r}
artists %>%  semi_join(songs)

artists %>% anti_join(bands)
```
...
```{r}
albums %>%  semi_join(bands) %>% nrow()

```
```{r}
songs %>%  semi_join(labels) %>% nrow()
```
